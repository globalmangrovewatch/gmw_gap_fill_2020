...
class GenExtractSamplesCmds(PBPTGenQProcessToolCmds):
    ...
    def gen_command_info(self, **kwargs):
        for i in range(kwargs['n_sample_sets']):
            mng_train_smps_file = os.path.join(kwargs['samples_dir'], "mng_train_samples_{}.h5".format(i + 1))
            ...
            out_cls_file = os.path.join(kwargs['out_cls_dir'], 'sen2_gfill_opt_xgb_cls_trained_{}.mdl'.format(i + 1))
...
if __name__ == "__main__":
    ...
    create_tools = GenExtractSamplesCmds(cmd=script_cmd, sqlite_db_file="sen2_train_xgb_cls.db")
