...
class MergeGMWEdits(PBPTQProcessTool):
    ...
    def do_processing(self, **kwargs):
        ...
        bandDefns = []
        bandDefns.append(rsgislib.imagecalc.BandDefn('gmw', self.params['gmw_v2_img'], 1))
        bandDefns.append(rsgislib.imagecalc.BandDefn('gfill', self.params['gapfill_img'], 1))
        bandDefns.append(rsgislib.imagecalc.BandDefn('vmsk', self.params['s2vmsk_img'], 1))
        rsgislib.imagecalc.bandMath(self.params['out_file'], '(gfill==1)?1:(vmsk==0)&&(gmw==1)?1:0', 'KEA', rsgislib.TYPE_8UINT, bandDefns)
        rsgislib.rastergis.populateStats(self.params['out_file'], addclrtab=True, calcpyramids=True, ignorezero=True)


    def required_fields(self, **kwargs):
        return ["tile_img", "gmw_v2_img", "gapfill_img", "s2vmsk_img", "out_file"]
